<?php
	...
	setcookie("sID",$subjectID);
	require_once("../Database/database.php");
	$query1 = mysqli_query($connect,"SELECT * FROM forumSubjects WHERE id = '$subjectID'");
	$row1 = mysqli_fetch_row($query1);
	echo "<h1>$row1[1]</h1>";
	$query2 = mysqli_query($connect,"SELECT usertype FROM profile WHERE username = '$row1[2]'") or die (mysqli_error($connect));
	$row2 = mysqli_fetch_row($query2);
	if ($row2[0] == "Administrator"){
		$name = "System Administrator";
	}
	
	else if($row2[0]=="Lecturer" || $row2[0]=="Student Counselor"){
						$sql = mysqli_query($connect,"SELECT name_with_initials FROM lecturer WHERE national_id_number= '$row1[2]'") or die("sorry");
						$row = mysqli_fetch_row($sql);
						$name = $row[0];
	}
	
	else if($row2[0]=="Employer"){
						$sql = mysqli_query($connect,"SELECT name FROM employer WHERE national_id_number= '$row1[2]'") or die("sorry");
						$row = mysqli_fetch_row($sql);
						$name = $row[0];
	}
	else if($row2[0]=="Undergraduate"){
						$sql = mysqli_query($connect,"SELECT first_name,middle_name,surname FROM student WHERE national_id_number= '$row1[2]'" )or die("sorry");
						$row = mysqli_fetch_row($sql);
						$name = $row[0]." ".$row[1]." ".$row[2];
						
	}
	else if($row2[0]=="Recent Graduate"){
						$sql = mysqli_query($connect,"SELECT name_with_initials FROM recentgraduate WHERE national_id_number= '$row1[2]'") or die("sorry");
						$row = mysqli_fetch_row($sql);
						$name = $row[0];
						
	}
	
	echo "<h3>Posted by <font color='red'>$name</font></h3>";
	echo "<br><br>";
	
	$query1 = mysqli_query($connect,"SELECT * FROM forumReplies WHERE subjectID = '$subjectID'")or die (mysqli_error($connect));
	//thibe forum messages loading from forumreplies table
	while($row = mysqli_fetch_row($query1)){
		$id = $row[0];
		$user = $row[1];
		$reply =$row[2];
		$query3 = mysqli_query($connect,"SELECT userType FROM profile WHERE userName = '$user'");
		$uRow = mysqli_fetch_row($query3);
		$userType = $uRow[0];
		//echo "$userType";
		...
		else if($userType=='Lecturer' || $userType=='Student Counselor'){
		...
		elseif($userType=='Recent Graduate'){
		...
		echo "<b>$name</b>";
		echo "<p>$reply</p>";
		if($_COOKIE["userType"] == "SystemAdmin" or $_COOKIE["userName"] == "$user"){
			echo "<form method='post' action='deleteReply.php'><button class='btn btn-default' name='delete' value='$id'>Delete</button></form></td>";
		}
		echo "<h4>Reply From <font color='red'>$name</font></h4>";
	}
	
	echo "<form method='post' action='insertForumReply.php'>";
	echo "<textarea name='reply' rows=5 cols=100></textarea>";
	echo "<button class='btn btn-primary' type='submit' name='subjectID' value='$subjectID'>Reply</button>";
	echo "</form>";
	echo "</div>";
